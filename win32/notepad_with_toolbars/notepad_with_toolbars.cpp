...
void UpdateStatusBar(HWND hwnd) {
	...
	LPSTR pszText = static_cast<LPSTR>(GlobalAlloc(GMEM_FIXED, LOWORD(SendMessage(GetDlgItem(hwnd, IDC_MAIN_STATUSBAR), SB_GETTEXTLENGTH, (WPARAM)0, (LPARAM)0))+1));
	if (pszText != NULL) {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, char* lpCmdLine, int nCmdShow) {
	...
	wc.hIcon = static_cast<HICON>(LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_MYICON)));
	wc.hCursor = LoadCursor(NULL, IDC_ARROW);
	wc.hbrBackground = (HBRUSH)(COLOR_WINDOW+1);
	wc.lpszMenuName = MAKEINTRESOURCE(IDR_MYMENU);
	wc.lpszClassName = notepad_szClassName;
	wc.hIconSm = static_cast<HICON>(LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_MYICON), IMAGE_ICON, 16, 16, 0));
